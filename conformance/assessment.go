...
// AssessGetInfo resolves the highest stable advertised profile and evaluates
// the response against the immutable specification edition owned by that
// profile.
func AssessGetInfo(info protocol.AuthenticatorGetInfoResponse) Assessment {
	...
	return assessGetInfo(info, target)
}

// AssessGetInfoAgainst evaluates against an explicit canonical profile and
...
func AssessGetInfoAgainst(info protocol.AuthenticatorGetInfoResponse, target Target) (Assessment, error) {
	if !isCanonicalTarget(target) {
		return Assessment{}, failure.New(failure.CodeConformanceTargetInvalid,
	...
	return assessGetInfo(info, target), nil
}

func assessGetInfo(info protocol.AuthenticatorGetInfoResponse, target Target) Assessment {
